...
这是一个使用AI短剧生成系统创建的示例项目，展示了自定义场景基础图功能。
...
本项目展示了如何使用自定义场景基础图功能，让用户可以为特定场景提供自己的图片，而不是依赖AI生成。

### 使用方法

1. **在剧本中指定场景图**：在 `script.txt` 中添加 `场景图:` 字段
   ```
   ## 场景1：逼仄的贫民窟出租屋
   ...
...
2. **放置图片**：将图片文件放在项目的 `scenes/` 文件夹中

3. **运行生成**：系统会自动加载自定义图片，如果找不到则回退到AI生成

### 示例

本项目的 `script.txt` 已经配置了场景1和子场景1-1使用自定义基础图：
- `scenes/scene_001_base.png` - 场景1的基础图
- `scenes/scene_001_sub_001_base.png` - 子场景1-1的基础图

详细使用说明请查看 [`scenes/README.md`](scenes/README.md)。
